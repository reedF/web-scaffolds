...
public class SendRequestUtil {
     ...
     * */
    public static SettlementPkiApiResponse doURLSend(SettlementPkiApiRequest request) {
        SettlementPkiApiResponse response = null;
        String postContent = StringUtils.ReqFormat(PayResultReversalUtil.settlementPkiApiRequestToXml(request));
        String sbr = HttpClientBill99.doPost(BillConstant.URL, postContent);
        if (sbr.length() > 0) {
            String responseXML= StringUtils.ResFormat(sbr);
            response=PayResultReversalUtil.xmlToSettlementPkiApiResponse(responseXML);
        }
        return response;
    }
